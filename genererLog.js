// genererLog.js
// ---------------------------------------------
// Génère un fichier auth.log factice pour pouvoir
// tester le détecteur sans avoir de vrai serveur.
// ---------------------------------------------
...
// Formate une date comme dans un vrai syslog : "Jul 21 09:15:32"
...
function genererLigne(date, ip, utilisateur, succes) {
  const action = succes ? 'Accepted' : 'Failed';
...
function main() {
  const lignes = [];
  let t = new Date(Date.now() - 2 * 60 * 60 * 1000); // il y a 2h
  ...
  for (const ip of ipsAttaquantes) {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      lignes.push({ date: temps, ligne: genererLigne(temps, ip, user, false) });
    }
  }

  // --- Simule du trafic normal : peu d'échecs puis un succès ---
